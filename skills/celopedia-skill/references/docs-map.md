...
> Moved from `tooling/overview/fee-abstraction` (removed) to its own subsection under `build-on-celo`.
...
> "Token Contracts" was renamed to **Stablecoin Contracts** (`tooling/contracts/token-contracts` → `tooling/contracts/stablecoin-contracts`). "Fee Currencies" is a new page listing the governable gas-fee-currency allowlist separately from stablecoin addresses.
