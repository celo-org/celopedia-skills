...
> Uniswap V3 has no Celo Sepolia deployment — only V4 (above) is on Celo Sepolia testnet. The Alfajores V3 addresses previously listed here have been removed: Alfajores (and Baklava) sunset in 2025, so those addresses are stale and should not be used.
